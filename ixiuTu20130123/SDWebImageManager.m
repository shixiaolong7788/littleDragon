...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate withIndex:(NSString *)index
{
    SDWebImageOptions options = 0;
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:delegate, @"delegate", url, @"url", [NSNumber numberWithInt:options], @"options",index, @"index",nil];
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([delegate respondsToSelector:@selector(webImageManagerIndex:didFinishWithImage:)])
    {
        [delegate performSelector:@selector(webImageManagerIndex:didFinishWithImage:) withObject:index withObject:image];
    }
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    NSString *index = [downloader.userInfo objectForKey:@"index"];
